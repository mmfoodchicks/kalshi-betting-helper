...
def preset_records():
    ...
    with _lock, _conn() as c:
        ...
        if "tag" not in cols:
            return {}
        rows = c.execute(
            "SELECT tag, date, graded, won, prob, n_legs, legs_hit "
            "FROM slip_log WHERE tag IS NOT NULL").fetchall()
    out, days = {}, {}
    for r in rows:
        d = out.setdefault(r["tag"], {"logged": 0, "graded": 0, "won": 0,
                                      "expected": 0.0, "void": 0,
                                      "legs": 0, "legs_hit": 0, "days": 0})
        d["logged"] += 1
        if r["graded"] == 1:
            d["graded"] += 1
            # Distinct graded DAYS: a lineup-churn afternoon can log the
            # same recipe three times with overlapping legs, and those
            # are one day of evidence, not three. The crown weights the
            # record by days so churn cannot buy it a say it hasn't earned.
            days.setdefault(r["tag"], set()).add(r["date"])
            d["won"] += r["won"] or 0
